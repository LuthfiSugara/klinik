...
import { addAppointment, getQueueAppointment } from '../redux/actions/appointment-actions';
...
const ChooseDoctor = ({navigation, route}) => {
    ...
    const redirectToQueue = () => {
        navigation.navigate('Queue', {
            "id_dokter" : detail_dokter.id,
            "date" : values.tanggal_berkunjung + ' ' + values.jam_berkunjung +":00"
        });
    }

    const submit = async() => {
        let queue = await dispatch(getQueueAppointment(
            detail_dokter.id, 
            values.tanggal_berkunjung + ' ' + values.jam_berkunjung +":00"
        ));

        if(queue.data.length < 10){
            handleSubmit();
        }else{
            Alert.alert(
                "Antrian sudah penuh",
                "Silahkan pilih tanggal dan jam lain",
            );
        }
    }
            ...
            <ScrollView style={tw`mb-20`}>
                <View>
                    ...
                    <View style={tw`bg-gray-200 p-4`}>
                        ...
                        <TouchableOpacity 
                            style={tw`bg-green-600 rounded`}
                            onPress={redirectToQueue}
                        >
                            <Text style={tw`text-white p-2 text-center`}>Lihat Antrian</Text>
                        </TouchableOpacity>
                    </View>
                ...
                <TouchableOpacity onPress={submit} style={tw`bg-red-500 m-4 p-3 rounded-lg mt-8`}>
